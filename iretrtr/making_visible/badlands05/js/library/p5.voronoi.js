...
(function() {
	...
	var imgHeight;
	//terra acqua
	var bw = [255, 255, 255, 255, '#2471A3'];
	var water = '#87bdd8';
	var land = 'hsba(0, 0%, 100%, 0)';
	var deepWater = '#87bdd8'
	...
	p5.prototype.voronoiSites = function(newSites){
		for (var i = 0; i < newSites.length; i++) {
			...
			if(newSites[i][2] !== undefined)
				...
				cellColors.push([newSites[i][0],newSites[i][1],color(random(bw))]);
			}
	}
	...
	*/
	p5.prototype.voronoiSite = function(nx, ny, nColor){
		...
		if(nColor !== undefined)
			...
			cellColors.push([nx,ny,color((random(bw)))]);
	}
	...
	*/
	...
	function setRandoms(width, height){
		for (var i = 0; i < nRandoms; i++) {
			...
			//Set Color
			if(dist(nX, nY, width/2, height/2) > height/2-random(height/4) && dist(nX, nY, width/2, height/2) < height-100){
			//if (nX < 50 || nY < 50 || nX > (width-50) || nY > (height-50)){
				cellColors.push([nX,nY,color(water), 'water']);
			}
			else if(dist(nX, nY, width/2, height/2) > height-100){
			//if (nX < 50 || nY < 50 || nX > (width-50) || nY > (height-50)){
				cellColors.push([nX,nY,color(deepWater), 'water']);
			}
			else {
				cellColors.push([nX,nY,color(land), 'land']);
			}
	...
	//Draw a voronoi Cell
	p5.prototype.voronoiDrawCell = function(x, y, id, newColor, type, fill = true, jitter = false){
	...
	//Set fill color from cell
	function changeColorCell(cellId, changeColor){
		for (var c = 0; c < cellColors.length; c++) {
			if(cellColors[c][0] == voronoiDiagram.cells[cellId].site.x && cellColors[c][1] == voronoiDiagram.cells[cellId].site.y){
				fill(changeColor);
			}
